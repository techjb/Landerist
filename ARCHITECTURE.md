...
`landerist_console/Program.cs` is only the process bootstrapper. Service registration enters through `AddLanderist`, while `LanderistServiceComposition` remains the temporary manual composition root during the migration.

Database, proxy, browser, Chrome maintenance, and execution-role settings cross that boundary as a validated `LanderistRuntimeOptions` model owned by Infrastructure. `LanderistRuntimeOptionsAdapter` is the compatibility edge that translates the legacy static configuration. New composition code must consume typed runtime options instead of reading `Config` or `LanderistSettings` directly.
## Boundaries
