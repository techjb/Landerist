...
`landerist_infrastructure` is the incremental destination for adapters. It currently owns HTTP transport, browser maintenance and the decoupled website adapters and the SQL connection abstraction, Page/Website/Listing persistence, page services and statistics repositories, and depends inward on `landerist_application` and `landerist_domain`; remaining adapters stay in `landerist_library/Infrastructure` until their legacy dependencies are removed.
## Boundaries
