using System.Text.RegularExpressions;

namespace landerist_architecture_tests;

public sealed class WebsitePolicyArchitectureTests
{
    [Fact]
    public void WebsitePolicyFiles_DoNotReadGlobalConfig()
    {
        string websitesRoot = Path.Combine(
            FindRepositoryRoot(),
            "landerist_library",
            "Websites");
        string[] policyFiles =
        [
            "Website.cs",
            "WebsiteHttpRequestProfile.cs",
            "IWebsiteRobotsPolicy.cs"
        ];
        string[] violations = policyFiles
            .Where(file => Regex.IsMatch(
                File.ReadAllText(Path.Combine(websitesRoot, file)),
                @"\bConfig\.",
                RegexOptions.CultureInvariant))
            .ToArray();

        Assert.True(
            violations.Length == 0,
            "Website policies must receive rules explicitly instead of reading Config." +
            Environment.NewLine +
            string.Join(Environment.NewLine, violations));
    }

    private static string FindRepositoryRoot()
    {
        DirectoryInfo? directory = new(AppContext.BaseDirectory);
        while (directory is not null)
        {
            if (File.Exists(Path.Combine(directory.FullName, "Landerist.sln")))
            {
                return directory.FullName;
            }

            directory = directory.Parent;
        }

        throw new DirectoryNotFoundException(
            "Could not locate the repository root containing Landerist.sln.");
    }
}
