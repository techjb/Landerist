...
namespace landerist_library.Database
{
    public class Backup
    {
        ...
        private static bool UploadBackup(string fileName)
        {
            ...
            return new S3().UploadFile(filePath, keyName, Config.AWS_S3_BUCKET_BACKUPS);
        }

        private static async void DeleteOldBackups()
        {
            Console.WriteLine("Deletings old backups ..");
            var S3Objects = await new S3().ListObjects(Config.AWS_S3_BUCKET_BACKUPS);
            ...
            var deletedObjects = await new S3().DeleteObjects(Config.AWS_S3_BUCKET_BACKUPS, toDelete);
